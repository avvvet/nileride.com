...
var setDriverVerify = (email,verify_type, user_type, status, callback) => {
    ...
    return sequelize.transaction(function (t) {
        return models.verifications.update(
            ...
            if(result){
               var verify = models.verifications.build(data);
               return verify.save().then((r)=> {
                    console.log('driver verification saved', r);
                    return r;
                  }, (err) => {
                     console.log('err', err);
                     throw new Error(err);
                ...
                throw new Error('driver old varification update err');
            }
        });
      
      }).then(function (result) {
          console.log('driver variication trsancation commited   tttttttttttttttttttttttttttttt ', result.dataValues);
          callback(result.dataValues.verification_token);
      }).catch(function (err) {
        console.log('driver varification trsancation rollback ', err);
      });
};
module.exports = {setUserVerify, setDriverVerify};